class Helpers {
    static getStateByName(options,context, stateStr) {
        ...
        return new State(context, state.nextState, state.callback, state.waitUntilNextState ?? 0);
    }
}
...
class Context {
    ...
    constructor(options) {
        this.currentState = Helpers.getStateByName(options,this, options.initialState)
        this.options=options;
    }

    transition() {
        if (this.currentState.nextState) {
            this.currentState = Helpers.getStateByName(this.options,this, this.currentState.nextState);
